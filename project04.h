...
enum scan_token_enum {
    ...
    TK_AND,
    TK_NOT,
    TK_XOR, //the result is 1 if the bits are different
    TK_EOT,    /* end of text */
...
enum parse_oper_enum {OP_PLUS, OP_MINUS, OP_MULT, OP_DIV, OP_NOT, OP_OR, OP_XOR, OP_AND, OP_SHIFT_LEFT, OP_SHIFT_RIGHT};

struct parse_node_st {
    enum parse_expr_enum type;
    union {
        struct {long value;} intval;
