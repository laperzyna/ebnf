...
char *scan_hexlit(struct scan_token_st *tp, char *p, char *end) {
  ...
  return scan_read_token(tp, p-len, len, TK_HEXLIT); 
}

char *scan_intORbin(struct scan_token_st *tp, char *p, char *end, enum scan_token_enum type) {
    // TODO: fill this in, like scan_read_token except just for integers
  int len = 0;
  while (scan_is_digit(*p) && (p != end)) {
    p++;
  ...
  return scan_read_token(tp, p-len, len, type); 
}
...
char * scan_token(char *p, char *end, struct scan_token_st *tp) {
    if (p == end) {
        ...
        p = scan_token(p, end, tp);
         ...
         if (*p == '0'){
            if (*(p+1) == 'b' ){
              p = scan_intORbin(tp, p+2, end, TK_BINLIT);
            } else if (*(p+1) == 'x'){
              p = scan_hexlit(tp,p+2,end);
            } else {
              p = scan_intORbin(tp, p, end, TK_INTLIT);
            }
        } else {   
          p = scan_intORbin(tp, p, end, TK_INTLIT);
        }
      ...
      p = scan_read_token(tp,p,2,TK_SHIFT_LEFT);
    }  else if (*p == '>' && *(p+1) == '>'){     
      p = scan_read_token(tp,p,2,TK_SHIFT_RIGHT);
    }   else if (*p == '|'){     
